    ...
    for qid in prev_results:
        ...
        if query_conf <= query_thres:
            def update_score(left, right):
                if right['doc_conf'] - left['doc_conf'] >= diff_thres:
                    right['score'] += 1
                    left['score'] -= 1
                elif left['doc_conf'] - right['doc_conf'] >= diff_thres:
                    right['score'] -= 1
                    left['score'] += 1
...
def eval_doc_diff_score_method(prev_results, measure_data, qrels):
