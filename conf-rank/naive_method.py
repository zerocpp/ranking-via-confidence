...
def _rerank(prev_results, measure_data, query_thres):
    rerank_results = {}
    for qid in prev_results:
        ...
        if query_conf <= query_thres:
            docs = sorted(docs, key=lambda x: (-x['doc_conf'], -x['rank_score']))
...
def eval_naive_method(prev_results, measure_data, qrels):
    ...
    for query_thres in QUERY_CONF_THRESHOLDS:
        ...
        rerank_results = _rerank(prev_results, measure_data, query_thres)
        all_results[query_thres] = rerank_results
        score = eval_rank_results(rerank_results, qrels)
        all_scores[query_thres] = score
